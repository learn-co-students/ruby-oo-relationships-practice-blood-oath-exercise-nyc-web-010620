class Cult
    ...
    def average_age # Cult#average_age returns a Float that is the average age of this cult's followers
        BloodOath.all.select{|oaths| oaths.cult == self}.map{|enum| enum.follower.age}.inject(:+) / BloodOath.all.select{|oaths| oaths.cult == self}.length.to_f #Test & Convert to float
    end
